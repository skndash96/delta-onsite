...
find httpd > files.txt
readarray -n 1000 files < files.txt
...
for file in "${files[@]}"; do
  ...
  # mod=`stat -c %Y "$PWD/$file"`  # doesnt work why?
  size=`stat -c %s "$PWD/$file"`
  mod=`stat -c %y "$PWD/$file"`
  urwx=`stat -c %A "$PWD/$file"`
  octal=`stat -c %a "$PWD/$file"`
  ym=${mod:0:7}
...
for ym in "${!yms[@]}"; do
  echo "$ym: ${yms[$ym]}" >> $YMS_FILE
  echo "${yms_array[$ym]}" >> $YMS_FILE
  echo "" >> $YMS_FILE
done
...
cat $YMS_FILE | sort -k2 -n -r | head -n 10 >> $OUT_FILE
